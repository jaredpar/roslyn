...
public sealed class CommandLineProjectWorkspaceTests
{
    ...
    {
        var projectDirectory = TestPathUtil.GetRootedPath("ProjectDirectory");
        var commandLine = $"goo.cs {Path.Combine("subdir", "bar.cs")} /out:goo.dll /target:library";
        var info = CommandLineProject.CreateProjectInfo("TestProject", LanguageNames.CSharp, commandLine, projectDirectory);
        ...
        Assert.Equal(0, gooDoc.Folders.Count);
        Assert.Equal(Path.Combine(projectDirectory, "goo.cs"), gooDoc.FilePath);
        ...
        Assert.Equal(Path.Combine(projectDirectory, "subdir", "bar.cs"), barDoc.FilePath);
    }
}
