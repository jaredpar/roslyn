...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal partial class Binder
    {
        ...
        /// </summary>
        internal const uint CallingMethod = 0;
        internal const uint ReturnOnlyScope = 1;
        internal const uint ContainingMethod = 2;
...
#nullable disable
        ...
        private bool CheckLocalRefEscape(SyntaxNode node, BoundLocal local, uint escapeTo, bool checkingReceiver, BindingDiagnosticBag diagnostics)
        {
            ...
            if (escapeTo is Binder.CallingMethod or Binder.ReturnOnlyScope)
            {
                if (localSymbol.RefKind == RefKind.None)
                {
                    if (checkingReceiver)
        ...
        private uint GetParameterValEscape(ParameterSymbol parameter)
        {
            if (UseUpdatedEscapeRules)
            {
                return parameter.EffectiveScope == DeclarationScope.ValueScoped ?
                    Binder.ContainingMethod :
                    Binder.CallingMethod;
            }
            else
            {
                return Binder.CallingMethod;
            }
        }
        ...
        private uint GetParameterRefEscape(ParameterSymbol parameter)
        {
            ...
            {
                { RefKind: RefKind.None } or { EffectiveScope: not DeclarationScope.Unscoped } => Binder.ContainingMethod,
                { Type.IsRefLikeType: true } => Binder.ReturnOnlyScope,
                _ => Binder.CallingMethod
            };
        }
        ...
        {
            Debug.Assert(escapeTo is Binder.CallingMethod or Binder.ReturnOnlyScope);
            if (UseUpdatedEscapeRules)
            {
...
#nullable enable
        ...
        )
        {
            ...
            uint escapeScope = Binder.CallingMethod;
...
#nullable disable
        ...
        internal uint GetRefEscape(BoundExpression expr, uint scopeOfTheContainingExpression)
        {
            ...
            switch (expr.Kind)
            {
                ...
                case BoundKind.RefValueOperator:
                    ...
                    return Binder.ContainingMethod;
        ...
        internal uint GetValEscape(BoundExpression expr, uint scopeOfTheContainingExpression)
        {
            ...
            switch (expr.Kind)
            {
                ...
                case BoundKind.RangeExpression:
                    ...
                    return Math.Max((range.LeftOperandOpt is { } left ? GetValEscape(left, scopeOfTheContainingExpression) : Binder.CallingMethod),
                                    (range.RightOperandOpt is { } right ? GetValEscape(right, scopeOfTheContainingExpression) : Binder.CallingMethod));
        ...
        {
            var result = Binder.CallingMethod;
        ...
        internal bool CheckValEscape(SyntaxNode node, BoundExpression expr, uint escapeFrom, uint escapeTo, bool checkingReceiver, BindingDiagnosticBag diagnostics)
        {
            ...
            switch (expr.Kind)
            {
                ...
                case BoundKind.StackAllocArrayCreation:
                case BoundKind.ConvertedStackAllocExpression:
                    if (escapeTo < Binder.ContainingMethod)
                    {
