...
namespace Microsoft.CodeAnalysis.CompilerServer
{
    internal sealed class NamedPipeClientConnectionHost : IClientConnectionHost
    {
            ...
            try
            {
                ...
                if (_listenTask is object && !_listenTask.IsCompleted)
                {
                    ...
                    catch
                    {
                        ...
                        // can be OperationCancelled when we connection was incomplete before
                        // the above cancel call or it could be 
                    }
                }
            }
            finally
        ...
        public Task<IClientConnection> GetNextClientConnectionAsync()
        {
            ...
            _listenTask = Task.Run(() => ListenCoreAsync(_pipeName, clientLoggingIdentifier, _cancellationTokenSource.Token));
            return _listenTask;
        }
