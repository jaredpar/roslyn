        ...
        Friend Overloads Shared Sub Analyze(info As FlowAnalysisInfo, region As FlowAnalysisRegionInfo,
            ...
            Try
                If walker.Analyze() Then
                    readInside = walker._readInside.Cast(Of ISymbol).ToImmutableArray()
                    writtenInside = walker._writtenInside.Cast(Of ISymbol).ToImmutableArray()
                    readOutside = walker._readOutside.Cast(Of ISymbol).ToImmutableArray()
                    writtenOutside = walker._writtenOutside.Cast(Of ISymbol).ToImmutableArray()
                    captured = walker._captured.Cast(Of ISymbol).ToImmutableArray()
                Else
