...
// TODO: delete this, it's an unnecessary abstraction
...
// TODO: get ExecutionTime out of here. No purpose
...
/// <summary>
/// Represents the work done by a single helix work item.
/// </summary>
internal readonly struct WorkItemInfo(ImmutableSortedDictionary<AssemblyInfo, ImmutableArray<TestMethodInfo>> filters, int partitionIndex)
{
    public int PartitionIndex { get; } = partitionIndex;
    public ImmutableSortedDictionary<AssemblyInfo, ImmutableArray<TestMethodInfo>> Filters { get; } = filters;
...
internal sealed class AssemblyScheduler
{
    ...
    {
        ConsoleUtil.WriteLine($"Scheduling {assemblies.Length} assemblies");
        ...
        Func<TestMethodInfo, TWeight, TWeight> addFunc) where TWeight : struct
    {
        ...
        {
            ConsoleUtil.WriteLine($"Building single assembly work item {workItemIndex} for {assembly.AssemblyPath}");
    ...
    // TODO: put logging in a separate place. Dump it to disk so that we can upload it 
    private static void LogWorkItems(ImmutableArray<WorkItemInfo> workItems)
    {
        ...
        foreach (var workItem in workItems)
        {
            ...
            ConsoleUtil.WriteLine($"- Work Item {workItem.PartitionIndex} (Execution time {totalExecutionTime})");
