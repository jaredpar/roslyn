...
internal static class HelixUtil
{
    ...
    internal static void GenerateHelixArtifacts(
        ...
        string helixArchitecture,
        OSPlatform? helixPlatform,
        ...
        ImmutableArray<WorkItemInfo> workItems)
    {
        ...
        if (helixPlatform is null)
        {
            throw new Exception($"{nameof(helixPlatform)} must be set to generate Helix artifacts");
        }
        ...
        var helixDirectory = Path.Combine(artifactsDirectory, "testPayload", ".helix", helixJobName);
        var workItemsDirectory = Path.Combine(helixDirectory, "workitems");
        _ = Directory.CreateDirectory(workItemsDirectory);
        ConsoleUtil.WriteLine($"Generating Helix artifacts in {helixDirectory}");
        if (Directory.Exists(helixDirectory))
        {
            ConsoleUtil.Warning($"Deleting existing Helix artifacts");
            Directory.Delete(helixDirectory, true);
        }
        ...
        if (helixPlatform == OSPlatform.Windows)
        {
            File.WriteAllText(Path.Combine(helixDirectory, "send-to-helix.ps1"), $"""
                . (Join-Path $PSScriptRoot "..\..\..\..\eng\common\tools.ps1")
                $dotnetRoot = Join-Path $PSScriptRoot ".dotnet"
                InstallDotNetSdk $dotnetRoot {dotnetSdkVersion}
                & $dotnetRoot\dotnet build helix.csproj -bl:helix.binlog
                """);
        }
        else
        {
            ...
            string architecture)
        {
            ...
            workItemInfo = new WorkItemInfo(
                workItemInfo.Filters.ToImmutableSortedDictionary(kvp => kvp.Key with { AssemblyPath = GetAssemblyPathInHelix(kvp.Key.AssemblyPath) }, kvp => kvp.Value),
                workItemInfo.PartitionIndex);
            var payloadDirectory = Path.Combine(workItemsDirectory, workItemInfo.PartitionIndex.ToString());
            var payloadRelativeDirectory = Path.Combine(Path.GetFileName(workItemsDirectory), workItemInfo.PartitionIndex.ToString());
            ...
            return $"""
                        <HelixWorkItem Include="{workItemInfo.DisplayName}">
                            <PayloadDirectory>{payloadRelativeDirectory}</PayloadDirectory>
